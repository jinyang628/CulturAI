...
export default function Riddle() {
  ...
  const [isLoadingRiddleHint, setIsLoadingRiddleHint] =
    useState<boolean>(false);
  const [isLoadingValidateImage, setIsLoadingValidateImage] =
    useState<boolean>(false);
  const [isLoadingTransformImage, setIsLoadingTransformImage] =
    useState<boolean>(false);
  ...
  const handleValidateImage = async (
    ...
    if (image) {
      ...
      try {
        setIsLoadingValidateImage(true)
        const response = await validateImage(formData);
        setIsLoadingValidateImage(false)
        setSubmittedValidation(true);
        setIsValidImage(response?.data);
      } catch (error) {
  ...
  const handleTransformImage = async ({
    ...
    if (image) {
      ...
      try {
        setIsLoadingTransformImage(true)
        const response = await transformImage(formData);
        setIsLoadingTransformImage(false)
  ...
  return (
    <div className="w-full flex flex-col gap-5 ">
      ...
      <div className="justify-center items-center w-full flex flex-col gap-4">
        <div className="min-w-[350px] text-left border-[0.5px] py-8 px-2 rounded-lg ">
        ...
        <div className="w-full flex justify-center items-center flex-col gap-5 mt-8">
          ...
          {uploadedValidationImage && isValidImage && submittedValidation && (
            <div
          ...
          {uploadedValidationImage && !isValidImage && submittedValidation && (
            ...
            >
              ...
              <span className="sr-only">Info</span>
              <div>
                <span className="font-medium">Incorrect image. Try again!</span>
              </div>
            </div>
          )}
          ...
          <input
            ...
            placeholder="Enter a description to transform the uploaded image"
          />
          {transformedImage && <img src={transformedImage} alt="Transformed" />}

          {isLoadingTransformImage ? (
            <div role="status">
              <svg
                aria-hidden="true"
                className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
              ...
              <button
                onClick={() => {
                  handleTransformImage({
                    ...
                    image: uploadedValidationImage,
                  });
                  console.log(textForTransformation);
                }}
                className="btn btn-primary text-white disabled:text-gray-700 disabled:border-white"
